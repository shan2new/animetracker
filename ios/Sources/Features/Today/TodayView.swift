...
struct TodayView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            .overlay(alignment: .top) { brandHeader.padding(.top, 10) }
            .refreshable {
    ...
    private var allCaughtUpBody: String {
        if let f = appModel.nextUp, let next = f.releasingPart?.nextAiringAt {
            // Anime lands at a clock time; TV lands on a day (its clock is synthesized).
            let label = f.source == .tmdb
                ? Formatting.fmtDayLong(ts: next, now: now)
                : "\(Formatting.fmtDay(ts: next, now: now)) \(Formatting.fmtTime(next))"
    ...
    private func heroMeta(_ f: Franchise, vm: CardModel, variant: HeroVariant) -> some View {
        ...
        case .newEpisode: newEpisodeMeta(vm)
        case .resume: resumeMeta(f)
        case .waiting: waitingMeta(vm)
        }
    ...
    private func newEpisodeMeta(_ vm: CardModel) -> (String, String) {
        ...
        if vm.source == .tmdb && vm.progress == 0 && vm.airedEpisodes > 1 {
            ...
            return (label.isEmpty ? "NEW SEASON" : label, "NOW STREAMING")
        }
    ...
    private func waitingMeta(_ vm: CardModel) -> (String, String) {
        var base = [vm.seasonToken(), vm.nextEp.map { "E\($0)" } ?? ""]
            .filter { !$0.isEmpty }.joined(separator: " ")
        let day = vm.dayWordLong.uppercased()
        if !day.isEmpty {
            base += base.isEmpty ? day : " · \(day)"
            if vm.source == .anilist && !vm.airTime.isEmpty { base += ", \(vm.airTime)" }
        }
        let clock = vm.relClock
        return (base, clock.isEmpty ? "" : (clock == "now" ? "NOW" : "IN \(clock.uppercased())"))
    }
    ...
    private func shelfCaption(_ f: Franchise, state: AppModel.ShelfState?) -> some View {
        ...
        case .airingWait:
            if let next = f.releasingPart?.nextAiringAt {
                let day = Formatting.fmtDay(ts: next, now: now)
                let span = Formatting.fmtRelSpanShort(ts: next, now: now)
                caption(day == "Today" ? "Today" : "\(day) · \(span)", color: Theme.accent)
            }
        case .premiereSoon:
            if let premiere = appModel.nextPremiere(of: f) {
                caption(Formatting.fmtMonthDay(premiere), color: Theme.text46)
            }
        case nil:
            EmptyView()
        }
